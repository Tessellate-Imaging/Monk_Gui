...
class WindowObj2PytorchFinetune(QtWidgets.QWidget):
    ...
    def __init__(self):
        super().__init__()
        self.title = 'Object Detection - Pytorch Finetune'
        self.left = 100
        self.top = 100
        self.width = 800
        self.height = 600
    ...
    def install(self):
        ...
        if(self.cb4.currentText() == "Cuda"):
            os.system("cp cfg/detection/object_detection/obj_2_pytorch_finetune/install_obj_2_pytorch_finetune_cuda.sh .");
            os.system("chmod +x install_obj_2_pytorch_finetune_cuda.sh");
            self.process.start('bash', ['install_obj_2_pytorch_finetune_cuda.sh'])
...
screen = WindowObj2PytorchFinetune()
screen.show()
sys.exit(app.exec_())
'''