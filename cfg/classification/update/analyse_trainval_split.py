...
analysis_name = system["analysis"]["trainval_split"]["analysis_name"];

# Batch sizes to explore    
trainval_splits = system["analysis"]["trainval_split"]["list"].split(",");
trainval_splits = list(map(float, trainval_splits))

# Num epochs for each experiment to run 
epochs=int(system["analysis"]["trainval_split"]["epochs"]);

# Percentage of original dataset to take in for experimentation
percent_data=int(system["analysis"]["trainval_split"]["percent"]);

# "keep_all" - Keep all the sub experiments created
# "keep_non" - Delete all sub experiments created   
analysis = ptf.Analyse_Trainval_Splits(analysis_name, 
                                        trainval_splits, 
                                        percent_data, 
                                        num_epochs=epochs, 
                                        state="keep_none"); 
