...
analysis_name = system["analysis"]["optimizer_lr"]["analysis_name"];

   
tmp_list = system["analysis"]["optimizer_lr"]["list"].split(",");
optimizers_lrs = [];
for i in range(len(tmp_list)//2):
    optimizers_lrs.append([tmp_list[2*i].replace(" ", ""), float(tmp_list[2*i+1].replace(" ", ""))])


epochs=int(system["analysis"]["optimizer_lr"]["epochs"]);


percent_data=int(system["analysis"]["optimizer_lr"]["percent"]);

 
analysis = ptf.Analyse_Optimizers_With_LR(analysis_name, 
                                    optimizers_lrs, 
                                    percent_data, 
                                    num_epochs=epochs, 
                                    state="keep_none"); 
