...
analysis_name = system["analysis"]["batch_size"]["analysis_name"];

# Batch sizes to explore    
batch_sizes = system["analysis"]["batch_size"]["list"].split(",");
batch_sizes = list(map(int, batch_sizes))

# Num epochs for each experiment to run 
epochs=int(system["analysis"]["batch_size"]["epochs"]);

# Percentage of original dataset to take in for experimentation
percent_data=int(system["analysis"]["batch_size"]["percent"]);

# "keep_all" - Keep all the sub experiments created
# "keep_non" - Delete all sub experiments created   
analysis = ptf.Analyse_Batch_Sizes(analysis_name, 
                                    batch_sizes, 
                                    percent_data, 
                                    num_epochs=epochs, 
                                    state="keep_none"); 
